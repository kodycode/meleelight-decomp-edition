    ...
    if (!puff.SIDESPECIALGROUND.interrupt(p, input)) {
      ...
      if (player[p].phys.grounded) {
        if (player[p].timer > 11) {
          setGroundVelocity(p, puff.SIDESPECIALGROUND.groundVelocities[player[p].timer - 12] * player[p].phys.face);
        }
      }
      else {
        ...
        if (player[p].timer < 12) {
          ...
          // Same recurrence as the aerial branch -- see SIDESPECIALAIR.js.
          // ftPr_SpecialAirS_Phys (ftpurinspecials.c:97-125): seed from xF0
          // once, then decay by xF4 every frame including the seeding one.
          const attr = player[p].charAttributes;
          if (player[p].timer === 12) {
            const ang = player[p].phys.upbAngleMultiplier;
            setGroundVelocity(p, mul(attr.rolloutSpeed, mul(player[p].phys.face, cosf(ang))));
            player[p].phys.cVel.y = mul(attr.rolloutSpeed, sinf(ang));
          }
          setGroundVelocity(p, mul(getGroundVelocity(p), attr.rolloutDecay));
